...
import { $t as valueOrDefault, J as almostEquals, X as callback, ct as each, ht as getRelativePosition, k as _isPointInArea, zt as sign } from "./helpers.dataset-vGQsQj6P.js";
//#endregion
//#region node_modules/chartjs-plugin-zoom/dist/chartjs-plugin-zoom.esm.js
...
function directionEnabled(mode, dir, chart) {
	...
	else if (typeof mode === "function") return mode({ chart }).indexOf(dir) !== -1;
	return false;
}
...
function getEnabledScalesByPoint(options, point, chart) {
	...
	if (overScaleMode) {
		...
		for (const axis of ["x", "y"]) if (overScaleEnabled[axis]) {
			scaleEnabled[axis] = enabled[axis];
			enabled[axis] = false;
		}
	}
	if (scale && scaleEnabled[scale.axis]) return [scale];
	const enabledScales = [];
...
function linearZoomDelta(scale, zoom, center) {
	...
	return zoomDelta(getValueAtPoint(scale, center), scale.min, range, newRange);
}
function logarithmicZoomRange(scale, zoom, center) {
	const centerValue = getValueAtPoint(scale, center);
	if (centerValue === void 0) return {
		min: scale.min,
		max: scale.max
	};
	...
	const delta = zoomDelta(logCenter, logMin, logRange, logRange * (zoom - 1));
	return {
...
function updateRange(scale, { min, max }, limits, zoom = false) {
	...
	if (zoom === "pan" && (min < minLimit || max > maxLimit)) return true;
	const scaleRange = scale.max - scale.min;
	const range = zoom ? Math.max(max - min, minRange) : scaleRange;
	if (zoom && range === minRange && scaleRange <= minRange) return true;
...
function zoomLogarithmicScale(scale, zoom, center, limits) {
	return updateRange(scale, logarithmicZoomRange(scale, zoom, center), limits, true);
}
...
function panNumericalScale(scale, delta, limits, pan = false) {
	...
	const offset = OFFSETS[options.time && options.time.round] || 0;
...
var zoomRectFunctions = { default: zoomRectNumericalScale };
var panFunctions = {
...
function doZoom(scale, amount, center, limits) {
	callback(zoomFunctions[scale.type] || zoomFunctions.default, [
		scale,
		amount,
		center,
		limits
...
function doZoomRect(scale, from, to, limits) {
	callback(zoomRectFunctions[scale.type] || zoomRectFunctions.default, [
		scale,
		from,
		to,
		limits
...
function zoom(chart, amount, transition = "none", trigger = "api") {
	...
	const xEnabled = x !== 1;
	const yEnabled = y !== 1;
	each(getEnabledScalesByPoint(zoomOptions, focalPoint, chart) || chart.scales, function(scale) {
...
function panScale(scale, delta, limits, state) {
	...
	if (sign(storedDelta) === sign(delta)) delta += storedDelta;
	if (callback(panFunctions[scale.type] || panFunctions.default, [
		scale,
		delta,
		limits
	])) panDelta[scale.id] = 0;
	else panDelta[scale.id] = delta;
}
...
function getZoomedScaleBounds(chart) {
	const state = getState(chart);
	const scaleBounds = {};
	for (const scaleId of Object.keys(chart.scales)) scaleBounds[scaleId] = state.updatedScaleLimits[scaleId];
	return scaleBounds;
}
function isZoomedOrPanned(chart) {
	...
	for (const scaleId of Object.keys(chart.scales)) {
		...
		if (originalMin !== void 0 && chart.scales[scaleId].min !== originalMin) return true;
		if (originalMax !== void 0 && chart.scales[scaleId].max !== originalMax) return true;
	}
	return false;
}
...
function zoomStart(chart, event, zoomOptions) {
	...
	if (onZoomStart) {
		if (callback(onZoomStart, [{
			chart,
			event,
			point: getPointPosition(event, chart)
		}]) === false) {
...
function mouseDown(chart, event) {
	if (chart.legend) {
		if (_isPointInArea(getRelativePosition(event, chart), chart.legend)) return;
	}
	const state = getState(chart);
	const { pan: panOptions, zoom: zoomOptions = {} } = state.options;
	if (event.button !== 0 || keyPressed(getModifierKey(panOptions), event) || keyNotPressed(getModifierKey(zoomOptions.drag), event)) return callback(zoomOptions.onZoomRejected, [{
		chart,
		event
	}]);
...
function applyAspectRatio({ begin, end }, aspectRatio) {
	...
	if (ratio > aspectRatio) width = Math.sign(width) * Math.abs(height * aspectRatio);
	else if (ratio < aspectRatio) height = Math.sign(height) * Math.abs(width / aspectRatio);
	end.x = begin.x + width;
...
function getRelativePoints(chart, pointEvents, maintainAspectRatio) {
	...
	if (maintainAspectRatio) applyAspectRatio(points, chart.chartArea.width / chart.chartArea.height);
	return points;
}
function computeDragRect(chart, mode, pointEvents, maintainAspectRatio) {
	...
	if (xEnabled) applyMinMaxProps(rect, chart.chartArea, points, {
		min: "left",
		max: "right",
		prop: "x"
	});
	if (yEnabled) applyMinMaxProps(rect, chart.chartArea, points, {
		min: "top",
		max: "bottom",
		prop: "y"
	});
...
function addDebouncedHandler(chart, name, handler, delay) {
	if (handler) getState(chart).handlers[name] = debounce(() => callback(handler, [{ chart }]), delay);
}
...
function startPan(chart, state, event) {
	...
	if (callback(onPanStart, [{
		...
		event,
		point
	}]) === false) return callback(onPanRejected, [{
		chart,
		event
	}]);
...
function startHammer(chart, options) {
	...
	const mc = new import_hammer.default.Manager(canvas);
	if (zoomOptions && zoomOptions.pinch.enabled) {
		mc.add(new import_hammer.default.Pinch());
		mc.on("pinchstart", (e) => startPinch(chart, state, e));
		mc.on("pinch", (e) => handlePinch(chart, state, e));
		mc.on("pinchend", (e) => endPinch(chart, state, e));
	}
	if (panOptions && panOptions.enabled) {
		mc.add(new import_hammer.default.Pan({
...
function hammerOptionsChanged(oldOptions, newOptions) {
	...
	if (oldZoom?.zoom?.pinch?.enabled !== newZoom?.zoom?.pinch?.enabled) return true;
	if (oldPan?.enabled !== newPan?.enabled) return true;
	if (oldPan?.threshold !== newPan?.threshold) return true;
	return false;
}
var version = "2.2.0";
...
var plugin = {
	...
	start: function(chart, _args, options) {
		...
		if (import_hammer.default) startHammer(chart, options);
	...
	stop: function(chart) {
		...
		if (import_hammer.default) stopHammer(chart);
		removeState(chart);
	},
	panFunctions,
	zoomFunctions,
